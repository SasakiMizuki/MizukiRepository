...
void CTextView::OnInitialUpdate()
{
	...
	m_MyEdit.SetEventMask(m_MyEdit.GetEventMask() | ENM_CHANGE | ENM_SELCHANGE);
...
{
	DWORD dwFontSize = AfxGetApp()->GetProfileInt(_T("Settings"), _T("FontSize"), 10);
	CString strFontName = AfxGetApp()->GetProfileString(_T("Settings"), _T("FontName"), _T("‚l‚r ƒSƒVƒbƒN"));
	CHARFORMAT cf;
	cf.cbSize = sizeof(cf);
	...
	// Rich Edit 2.0 or later supports Unicode.
	StringCchCopy(cf.szFaceName, 32, strFontName);
	m_MyEdit.SendMessage(EM_SETLANGOPTIONS, 0, m_MyEdit.SendMessage(EM_GETLANGOPTIONS, 0, 0) & ~IMF_DUALFONT);
#else
	// Before 2.0, rich edit only supports ANSI.
	WideCharToMultiByte(CP_ACP, 0, strFontName, -1, cf.szFaceName, _countof(cf.szFaceName), NULL, NULL);
	cf.szFaceName[31] = 0;
#endif
	m_MyEdit.SetDefaultCharFormat(cf);
}

void CTextView::SyncContents()
{
	if (m_MyEdit.GetSafeHwnd() != NULL)
	{
		m_strEdit = ((CEffectDoc*)GetDocument())->GetCode();
		m_MyEdit.SetWindowText(m_strEdit);
		m_MyEdit.SetSel(0, 0);
	}
}
...
void CTextView::Locate(int nCol, int nRow)
{
	int nBegin = m_MyEdit.LineIndex(nRow) + nCol;
	m_MyEdit.SetSel(nBegin, nBegin);
}
...
void CTextView::OnEditFind()
{
    ...
    if (m_pFRDlg->m_hWnd == NULL)
        m_pFRDlg->Create(TRUE, _T(""), NULL, FR_DOWN, this);
    else
        m_pFRDlg->BringWindowToTop();
}

LRESULT CTextView::OnFindReplace(WPARAM wParam, LPARAM lParam)
{
    ...
    m_MyEdit.GetSel(FindTextEx.chrg);
    if (FindTextEx.chrg.cpMax != FindTextEx.chrg.cpMin)
        FindTextEx.chrg.cpMin++;
    FindTextEx.chrg.cpMax = m_MyEdit.GetTextLength() - 1;
#if (_RICHEDIT_VER >= 0x0200)
    // Rich Edit 2.0 or later supports Unicode.
    FindTextEx.lpstrText = pfr->lpstrFindWhat;
#else
    // Before 2.0, rich edit only supports ANSI.
    char strFindWhat[512];
    WideCharToMultiByte(CP_ACP, 0, pfr->lpstrFindWhat, -1, strFindWhat, 512, NULL, NULL);
    strFindWhat[511] = '\0';
    FindTextEx.lpstrText = strFindWhat;
#endif
	FindTextEx.chrgText = FindTextEx.chrg;
//	if (m_MyEdit.FindText(dwFindFlags, &FindTextEx) != -1)
	if (m_MyEdit.SendMessageW(EM_FINDTEXTEXW, dwFindFlags, (LPARAM)&FindTextEx) != -1)
    {
        m_MyEdit.SetSel(FindTextEx.chrgText);
    }
    return 0;
}
...
CMyRichEditCtrl::CMyRichEditCtrl()
{
    ...
    if (!::GetSystemDirectory(wszPath, MAX_PATH + 1))
        return;
    StringCchCat(wszPath, MAX_PATH, L"\\RICHED20.DLL");
    m_hRELibrary = LoadLibraryW(wszPath);
}
...
void CMyRichEditCtrl::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
    ...
    if (nChar == 9)
    {
        ...
        else
        {
			CHARRANGE cr;
			GetSel(cr);
			int iSpace = cr.cpMin - LineIndex(-1);
			for (iSpace %= numSpaces; iSpace < numSpaces; iSpace++)
				SendMessage(EM_REPLACESEL, TRUE, (LPARAM)" ");
        }
    }
    else
...
BOOL CTextView::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	if (pHdr->idFrom == IDC_EDIT && pHdr->code == EN_SELCHANGE)
	{
		// edit control's caret has moved. Find the new line and column numbers.
		SELCHANGE* pSelCh = (SELCHANGE*)lParam;

		// Note: We want the first line/column to be 1, not 0.
		int iRow = 1 + m_MyEdit.LineFromChar(pSelCh->chrg.cpMin);
		int iCol = 1 + pSelCh->chrg.cpMin - m_MyEdit.LineIndex(-1);

		((CMainFrame*)theApp.GetMainWnd())->SetRowCol(iRow, iCol);
	}
