...
// DXUT Callbacks
void CRenderView::OnFrameMoveCB(IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext)
{
    if (s_pRenderView)
        s_pRenderView->OnFrameMove(pd3dDevice, fTime, fElapsedTime);
}

void CRenderView::OnFrameRenderCB(IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext)
{
    if (s_pRenderView)
        s_pRenderView->OnFrameRender(pd3dDevice, fTime, fElapsedTime);
}

HRESULT CRenderView::OnCreateDeviceCB(IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext)
{
    if (s_pRenderView)
...
HRESULT CRenderView::OnResetDeviceCB(IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext)
{
    if (s_pRenderView)
...
void CRenderView::OnLostDeviceCB(void* pUserContext)
{
    if (s_pRenderView)
        s_pRenderView->OnLostDevice();
}

void CRenderView::OnDestroyDeviceCB(void* pUserContext)
{
    if (s_pRenderView)
...
void CRenderView::OnInitialUpdate()
{
    ...
    if (!DXUTGetD3DDevice())
    {
        ...
        {
            MessageBox(_T("EffectEdit2 開始時、レンダー ビューが可視である必要があります。ウィンドウ サイズかスプリッタを変更して EffectEdit2 を再起動してください。"),
                _T("EffectEdit2"));
        }
        else
        {
...
{
	const LPDIRECT3DBASETEXTURE9 pNullTexture = NULL;
    HRESULT hr;
    ...
    if (SUCCEEDED(hr = pd3dDevice->BeginScene()))
    {
        if (m_pEffect != NULL)
        {
            if (m_pEnvironmentTexture != NULL)
            {
                ...
                // Note: EffectEdit2 always shows the environment cubemap aligned with world
                    ...
                    for (UINT iPass = 0; iPass < numPasses; iPass++)
                    {
                        ...
                        else
                        {
                            for (UINT i = 0; i < m_dwNumMaterials; i++)
                            {
								...
								// MOD 2007-11-21 H.Hamaya
								if (m_MaterialEmissiveEffectHandle != NULL)
									m_pEffect->SetVector(m_MaterialEmissiveEffectHandle, (LPD3DXVECTOR4)&m_pMaterials[i].MatD3D.Emissive);
								if (!m_bNoTextures && m_ppTextures[i] != NULL)
								{
									if (m_MaterialTextureEffectHandle != NULL)
										m_pEffect->SetValue(m_MaterialTextureEffectHandle, (void*)&m_ppTextures[i], sizeof(LPDIRECT3DBASETEXTURE9));
									if (m_MaterialTextureValidEffectHandle != NULL)
										m_pEffect->SetBool(m_MaterialTextureValidEffectHandle, TRUE);
								}
								else
								{
									if (m_MaterialTextureEffectHandle != NULL)
										m_pEffect->SetValue(m_MaterialTextureEffectHandle, (void*)&pNullTexture, sizeof(LPDIRECT3DBASETEXTURE9));
									if (m_MaterialTextureValidEffectHandle != NULL)
										m_pEffect->SetBool(m_MaterialTextureValidEffectHandle, FALSE);
								}
        ...
        if (m_bShowStats)
        {
            ...
            if (m_pEffect && !m_bTechniqueValidValid)
            {
            ...
            if (!m_bTechniqueValid)
            {
                ...
                txtHelper.DrawTextLine(L"Warning: 現在のデバイス設定では無効なテクニックです");
            }
            ...
            if (!((CEffectEditApp*)AfxGetApp())->AppActivated())
            {
                ...
                txtHelper.DrawTextLine(L"Note: 最前面でないため、フレーム間で休止します。fps値は下がります");
            }
...
HRESULT CRenderView::OnCreateDevice(IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc)
{
    ...
    m_DialogResourceManager.OnCreateDevice(pd3dDevice);
    m_SettingsDlg.OnCreateDevice(pd3dDevice);
                                   ...
                                   CStringList& techniqueNameList, int* piTechnique, BOOL* pbTryLater)
{
	...
	*pbTryLater = FALSE;
	...
	m_MaterialEmissiveEffectHandle = NULL;		// ADD H.Hamaya
	m_MaterialTextureEffectHandle = NULL;		// ADD H.Hamaya
	m_MaterialTextureValidEffectHandle = NULL;	// ADD H.Hamaya
    ...
    GetDlgItem(IDC_RENDERTEXT)->SetWindowText(_T("(レンダリングするものがありません)"));
    ...
    if (!bFromFile)
    {
        ...
        char* strEffectA = new char[EffectLength * 2 + 1];
        if (strEffectA == NULL)
            return E_OUTOFMEMORY;
		ZeroMemory(strEffectA, EffectLength * 2 + 1);
        WideCharToMultiByte(CP_ACP, 0, strEffect, -1, strEffectA, EffectLength * 2 + 1, NULL, NULL);

        hr = D3DXCreateEffect(DXUTGetD3DDevice(), strEffectA, strlen(strEffectA), NULL,
    ...
    if (strErrors.IsEmpty() && SUCCEEDED(hr))
        strErrors = _T("エフェクトのコンパイルに成功しました\n");
...
HRESULT CRenderView::ParseParameters(CString& strErrors)
{
    ...
    for (UINT iParam = 0; iParam < m_EffectDesc.Parameters; iParam++)
    {
            ...
            ParamDesc.Class == D3DXPC_VECTOR)
        {
			...
			// ADD H.Hamaya
				...
				m_MaterialEmissiveEffectHandle = hParam;
        }
            ...
            ParamDesc.Class == D3DXPC_SCALAR)
        {
			...
			// ADD H.Hamaya
			else if (_strcmpi(ParamDesc.Semantic, "materialtexturevalid") == 0)
				m_MaterialTextureValidEffectHandle = hParam;
        }

		// ADD H.Hamaya
		if (ParamDesc.Semantic != NULL && ParamDesc.Class == D3DXPC_OBJECT)
		{
			if (_strcmpi(ParamDesc.Semantic, "materialtexture") == 0)
				m_MaterialTextureEffectHandle = hParam;
		}
        ...
        SUCCEEDED(m_pEffect->GetParameterDesc("XFile", &Desc)))
    {
        ...
        {
            UnloadMesh();
            StringCchCopyA(m_strMesh, MAX_PATH, fileName);
    ...
    if (bUsesTangents && m_pMesh != NULL)
    {
        ...
        if (!bHasTangents)
        {
            ...
            if (SUCCEEDED(hr))
            {
				...
				hr = D3DXComputeTangent(m_pMesh, 0, 0, D3DX_DEFAULT, FALSE, NULL);
            }
        }
    }
...
HRESULT CRenderView::LoadMesh(CString& strErrors)
{
	...
	WCHAR* pwszLastSlash = wcsrchr(strMeshPath, L'/');
	if (pwszLastBSlash || pwszLastSlash) {
		if (pwszLastBSlash > pwszLastSlash)
			*pwszLastBSlash = L'\0';
		else
			*pwszLastSlash = L'\0';
	} else
		StringCchCopy(strMeshPath, MAX_PATH, L".");
    ...
    for (UINT i = 0; i < m_dwNumMaterials; i++)
    {
        ...
        if (m_pMaterials[i].pTextureFilename != NULL && m_pMaterials[i].pTextureFilename[0] != '\0')
        {
...
LRESULT CRenderView::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if (m_SettingsDlg.IsActive())
    {
        ...
        {
            bProcessing = true;
            m_SettingsDlg.MsgProc(m_hWnd, message, wParam, lParam);
            bProcessing = false;
        }
    }
...
{
    CString str;
    str.Format(_T("テクニック '%s' によるレンダリング"), strTechName);
    m_iTechnique = iTech;
...
HRESULT CRenderView::EE_FindMediaFileCch(TCHAR* szDestPath, int cchDest, TCHAR* strFilename)
{
    ...
    CString strDocT;
    int iChar = strDoc.ReverseFind(_T('\\'));
    if (iChar >= 0)
    {
        strDoc = strDoc.Left(iChar);

        strDocT = strDoc + _T("\\") + strFilename;
        if (FileExists(strDocT))
        {
            return StringCchCopy(szDestPath, cchDest, strDocT);
        }

        strDocT = strDoc + _T("\\Media\\") + strFilename;
        if (FileExists(strDocT))
        {
            return StringCchCopy(szDestPath, cchDest, strDocT);
        }
