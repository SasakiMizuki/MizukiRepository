...
void CErrorsView::ParseErrors()
{
	...
	if (m_ListBox.GetCount() == 0)
		m_ListBox.AddString(_T("No errors"));
...
void CErrorsView::OnDblclkList() 
{
	...
	if (iColm < 0)
		return;
	str = str.Left(iColm);
	if (2 == _stscanf(str, _T("%d,%d"), &iLine, &iColm))
	{
		((CEffectEditApp*)AfxGetApp())->Locate(iColm - 1, iLine - 1);
