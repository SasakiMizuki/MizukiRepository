...
const char* g_strCodeDefault = 
	...
	"\r\n"
	"// ここには光源からの方向ベクトルを表すベクトル パラメータ\r\n"
	"// があります。UIDirectional アノテーションは、光源方向を\r\n"
	"// インタラクティブに変更することができるユーザ インタフェース\r\n"
	"// を作成するようにEffectEditに指示します。\r\n"
	"float3 lightDir\r\n"
	"<\r\n"
	"    string UIDirectional = \"Light Direction\";\r\n"
	"> = {0.577, -0.577, 0.577};\r\n"
	"\r\n"
	"string XFile = \"tiger\\\\tiger.x\";    // 読み込むモデル\r\n"
	"string BIMG  = \"misc\\\\lake.bmp\";    // 背景画像\r\n"
	"DWORD  BCLR  = 0xff202080;  // 背景色 (画像が無い場合)\r\n"
	"\r\n"
	"technique tec0\r\n"
	"{\r\n"
	"    pass p0\r\n"
	...
	"        LightDirection[0] = <lightDir>; // 上で定義されたベクトル パラメータを使用\r\n"
	...
	"\r\n"
	"        // テクスチャ ステージ 0 の設定\r\n"
	"        Texture[0] = <tex0>;    // 上で定義されたテクスチャ パラメータを使用\r\n"
	"        ColorOp[0] = Modulate;\r\n"
	"        ColorArg1[0] = Texture;\r\n"
	"        ColorArg2[0] = Diffuse;\r\n"
	"        AlphaOp[0] = Modulate;\r\n"
...
void CEffectDoc::OnEditUseExternalEditor()
{
	if (!m_bUsingExternalEditor)
	{
		...
		{
			if (IDCANCEL == MessageBox(GetTextView()->GetSafeHwnd(), _T("外部エディタ モードに移行する前に、このエフェクト ファイルを保存する必要があります。OK を押して保存してください。"), _T("EffectEdit2"), MB_OKCANCEL))
				return;
			if (!DoFileSave())
				return;
		}
