...
HRESULT CUIElements::OnCreateDevice(LPDIRECT3DDEVICE9 pd3dDevice)
{
    if (m_pd3dFont == NULL)
        D3DXCreateFont(pd3dDevice, -12, 0, FW_BOLD, 1, FALSE, DEFAULT_CHARSET,
                       OUT_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE,
                       _T("Arial"), &m_pd3dFont);

    D3DXLoadMeshFromXResource(NULL, (LPCSTR)MAKEINTRESOURCE(IDR_ARROW_X), "X", D3DXMESH_MANAGED,
        pd3dDevice, NULL, NULL, NULL, NULL, &m_pMeshArrow);
    D3DXCreateEffectFromResource(pd3dDevice, NULL, MAKEINTRESOURCE(IDR_UIELEMENT_FX),
        NULL, NULL, 0, NULL, &m_pEffect, NULL);

    return S_OK;
}

HRESULT CUIElements::OnResetDevice(LPDIRECT3DDEVICE9 pd3dDevice)
{
    if (m_pEffect != NULL)
...
void CUIElements::OnDestroyDevice()
{
    DeleteEffectObjects();
...
HRESULT CUIElements::Render(IDirect3DDevice9* pd3dDevice)
{
...
void CDirLight::Render(IDirect3DDevice9* pd3dDevice)
{
    ...
    if (m_bSelected)
    {
        ...
        RECT rc = {(LONG)vec.x + 1, (LONG)vec.y + 1, (LONG)vec.x + 1, (LONG)vec.y + 1};
        m_pUIElements->m_pd3dFont->DrawText(NULL, str, -1, &rc, DT_LEFT|DT_NOCLIP, 0xff000000);
        OffsetRect(&rc, -1, -1);
        m_pUIElements->m_pd3dFont->DrawText(NULL, str, -1, &rc, DT_LEFT|DT_NOCLIP, 0xffffffff);
    }
}
