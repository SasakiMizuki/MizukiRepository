...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    // Set up the row and column pane width in the status bar
	UINT nId, nStyle;
	int	nWidth;
	m_wndStatusBar.GetPaneInfo(1, nId, nStyle, nWidth);
    nWidth = 60;
	m_wndStatusBar.SetPaneInfo(1, nId, nStyle, nWidth);

	m_wndStatusBar.GetPaneInfo(2, nId, nStyle, nWidth);
    nWidth = 60;
	m_wndStatusBar.SetPaneInfo(2, nId, nStyle, nWidth);

    // Initialize the pane text to line 1 & col 1
    SetRowCol(1, 1);
...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext) 
{
    ...
    UINT wpSize;
    WINDOWPLACEMENT* pwp;
    if (AfxGetApp()->GetProfileBinary(_T("Settings"), _T("WindowPlacement"), (BYTE**)&pwp, &wpSize))
    {
    ...
    INT cx = AfxGetApp()->GetProfileInt(_T("Settings"), _T("LeftColumnWidth"), 400);
    INT cyTopLeft = AfxGetApp()->GetProfileInt(_T("Settings"), _T("TopLeftPaneHeight"), 500);
    INT cyTopRight = AfxGetApp()->GetProfileInt(_T("Settings"), _T("TopRightPaneHeight"), 400);
...
void CMainFrame::Locate(int nCol, int nRow)
{
    CTextView* pTextView = (CTextView*)m_wndSplitterLeft.GetPane(0, 0);
    pTextView->Locate(nCol, nRow);
}
...
void CMainFrame::OnClose() 
{
    ...
    AfxGetApp()->WriteProfileBinary(_T("Settings"), _T("WindowPlacement"), (BYTE*)&wp, wp.length);

    INT cxCur;
    INT cxMin;
    m_wndSplitterMain.GetColumnInfo(0, cxCur, cxMin);

    AfxGetApp()->WriteProfileInt(_T("Settings"), _T("LeftColumnWidth"), cxCur);

    INT cyCur;
    INT cyMin;
    m_wndSplitterLeft.GetRowInfo(0, cyCur, cyMin);
    AfxGetApp()->WriteProfileInt(_T("Settings"), _T("TopLeftPaneHeight"), cyCur);

    m_wndSplitterRight.GetRowInfo(0, cyCur, cyMin);
    AfxGetApp()->WriteProfileInt(_T("Settings"), _T("TopRightPaneHeight"), cyCur);
...
{
    ((CEffectEditApp*)AfxGetApp())->SetAppActivated(bActive ? true : false);
}

void CMainFrame::SetRowCol(int iRow, int iCol)
{
    TCHAR tszBuf[20];

    StringCchPrintf(tszBuf, 20, _T("Ln %d"), iRow);
	m_wndStatusBar.SetPaneText(1, tszBuf);
    StringCchPrintf(tszBuf, 20, _T("Col %d"), iCol);
	m_wndStatusBar.SetPaneText(2, tszBuf);
}
