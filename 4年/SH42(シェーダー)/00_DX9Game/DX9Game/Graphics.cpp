...
bool CGraphics::Initialize(HWND hWnd, int nWidth, int nHeight, bool bWindow)
{
	...
	//-------- ここから３Ｄ関連
	// ビューマトリックス設定
	m_vCamera = D3DXVECTOR3(0.0f, 100.0f, -300);
		//-(m_nHeight / 2) / tanf(D3DXToRadian(FOVY) / 2));
	m_vLookAt = D3DXVECTOR3(0.0f, 100.0f, 0.0f);
	...
	m_Light.Diffuse.r = m_Light.Diffuse.g = m_Light.Diffuse.b = m_Light.Diffuse.a = 0.8f;
