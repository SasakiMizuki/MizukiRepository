...
tShadow					g_aShadow[MAX_SHADOW];			// 影ワーク

//=============================================================================
// 初期化処理
//=============================================================================
HRESULT InitShadow(LPDIRECT3DDEVICE9 pDevice)
{
	// 頂点情報の作成
...
// 描画処理
//=============================================================================
void DrawShadow(LPDIRECT3DDEVICE9 pDevice)
{
	...
	for(int nCntShadow = 0; nCntShadow < MAX_SHADOW; nCntShadow++)
	{
		if( g_aShadow[nCntShadow].bUse != false )	continue;
...
{
	// オブジェクトの頂点バッファを生成
    if(FAILED(pDevice->CreateVertexBuffer(sizeof(VERTEX_3D) * ,	// 頂点データ用に確保するバッファサイズ(バイト単位)
...
int CreateShadow(D3DXVECTOR3 pos, float fSizeX, float fSizeZ)
{
	...
	for(int nCntShadow = 0; nCntShadow < MAX_SHADOW; nCntShadow++)
	{
		if( g_aShadow[nCntShadow].bUse == true )	continue;
