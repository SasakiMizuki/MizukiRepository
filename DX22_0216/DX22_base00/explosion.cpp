...
// マクロ定義
//*****************************************************************************
#define	TEXTURE_EXPLOSION		"data/TEXTURE/explosion001.png"	// 読み込むテクスチャファイル名
...
void UpdateExplosion(void)
{
	...
	for(int nCntExplosion = 0; nCntExplosion < MAX_EXPLOSION; nCntExplosion++){
		...
		{
			// 頂点カラーの設定
			g_aExplosion[nCntExplosion].fAlpha -= 0.02f;
			if(g_aExplosion[nCntExplosion].fAlpha < 0.0f)
			{
				g_aExplosion[nCntExplosion].fAlpha = 0.0f;
			}
			SetColorExplosion(nCntExplosion, D3DXCOLOR(1.0f, 1.0f, 1.0f, g_aExplosion[nCntExplosion].fAlpha));
		}
	}
}
...
void DrawExplosion(LPDIRECT3DDEVICE9 pDevice)
{
	...
	pDevice->SetRenderState(D3DRS_ZWRITEENABLE, FALSE);					// Zバッファ更新を無効に
	...
	for(int nCntExplosion = 0; nCntExplosion < MAX_EXPLOSION; nCntExplosion++)
	{
		...
		// スケールを反映
		D3DXMatrixScaling(&mtxScale, g_aExplosion[nCntExplosion].scale.x,
										g_aExplosion[nCntExplosion].scale.y,
										g_aExplosion[nCntExplosion].scale.z);
		D3DXMatrixMultiply(&g_aExplosion[nCntExplosion].mtxWorld, &g_aExplosion[nCntExplosion].mtxWorld, &mtxScale);

		// 移動を反映
		D3DXMatrixTranslation(&mtxTranslate, g_aExplosion[nCntExplosion].pos.x,
												g_aExplosion[nCntExplosion].pos.y,
												g_aExplosion[nCntExplosion].pos.z);
		D3DXMatrixMultiply(&g_aExplosion[nCntExplosion].mtxWorld, &g_aExplosion[nCntExplosion].mtxWorld, &mtxTranslate);
...
//	pDevice->SetRenderState(D3DRS_ALPHATESTENABLE, FALSE);
	...
	pDevice->SetRenderState(D3DRS_ZWRITEENABLE, TRUE);					// Zバッファ更新を有効に
...
void SetTextureExplosion(int nIdxExplosion, int nPattern)
{
	{//頂点バッファの中身を埋める
		...
		// 頂点座標の設定
		pVtx[3].tex = D3DXVECTOR2(nPattern * 0.125f, 0.0f);
		pVtx[1].tex = D3DXVECTOR2((nPattern + 1) * 0.125f, 0.0f);
		pVtx[2].tex = D3DXVECTOR2(nPattern * 0.125f, 1.0f);
		pVtx[0].tex = D3DXVECTOR2((nPattern + 1) * 0.125f, 1.0f);
