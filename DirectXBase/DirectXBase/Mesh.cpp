...
// 境界ボックス描画
void CMesh::DrawOBB(D3DXMATRIX& world) {
	// 中心座標へ移動
	D3DXMATRIX mTrans;
	...
	pDevice->SetTransform(D3DTS_WORLD, &(mTrans * world));
...
// 線分と三角形の当たり判定
bool CMesh::Intersect(D3DXVECTOR3& L0,
	...
	D3DXMATRIX* pWorld)
{
	...
	if (pWorld) {
		D3DXMATRIX mInv;
		D3DXMatrixInverse(&mInv, NULL, pWorld);
		D3DXVec3TransformCoord(&L, &L0, &mInv);
		D3DXVec3TransformNormal(&W, &W, &mInv);
	} else {
		L = L0;
	}
	...
	MVERTEX* pVtx;
	m_pD3DMesh->LockVertexBuffer(0, (LPVOID*)&pVtx);
	WORD* pIdx;
	m_pD3DMesh->LockIndexBuffer(0, (LPVOID*)&pIdx);
	DWORD dwIdx = m_pD3DMesh->GetNumFaces();
	for (DWORD i = 0; i < dwIdx; ++i) {
		// 三角形の頂点取得
		D3DXVECTOR3 P0 = pVtx[*pIdx++].pos;
		D3DXVECTOR3 P1 = pVtx[*pIdx++].pos;
		D3DXVECTOR3 P2 = pVtx[*pIdx++].pos;
		// 三角形の辺のベクトル取得
		...
		if (D3DXVec3Dot(&N, &(L - P0)) < 0.0f) {
			continue;
		}
		D3DXVec3Cross(&N, &P1P2, &W);
		if (D3DXVec3Dot(&N, &(L - P1)) < 0.0f) {
			continue;
		}
		D3DXVec3Cross(&N, &P2P0, &W);
		if (D3DXVec3Dot(&N, &(L - P2)) < 0.0f) {
			continue;
		}
		// 三角形の法線ベクトル取得、正規化
		...
		// tを求める
		float t = D3DXVec3Dot(&N, &(P0 - L)) / base;
		if (t < 0.0f) {
			continue;	// 交点が始点より後方
		}
		if (!bRay && t > 1.0f) {
		...
		if (pNormal) {
			if (pWorld) {
				D3DXVec3TransformNormal(pNormal,
					&N, pWorld);
			} else {
				*pNormal = N;
			}
